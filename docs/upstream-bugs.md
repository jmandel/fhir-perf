...
| [15](#15-simplehttpclient-percent-decodes-redirect-location-headers-breaking-signed-url-redirects-github-releases-s3azure-presigned) | P2 | `SimpleHTTPClient` percent-decodes redirect Location headers | Active-by-default, all-consumer radius: every download whose server redirects to a signed URL fails 400; found live and verified at the pinned SHA. |
...
**The bug:** Five distinct sources, all upstream of our changes (reproduced on stock builds):
...
5. **Output depends on the checkout directory (kindling).** Published pages embed the build's absolute local path: the page templates' htmldiff/Jira links interpolate `Utilities.URLEncode(webLocation)`, which for a local build resolves to `file:/<absolute checkout path>/publish/...` (link construction e.g. [PageProcessor.java#L922](https://github.com/HL7/kindling/blob/b6cb1f66e49ac8ef4ace2932ad5345cb3e5624d8/src/main/java/org/hl7/fhir/tools/publisher/PageProcessor.java#L922), plus the footer templates). Two builds of the same commit from different directories differ on essentially every page. Observed concretely: a fresh clone built in another directory differed from the original checkout's build on ~every `.html` file solely in these embedded paths.
...
**Status / recommendation.** Most of these are now **fixed in the author's forks** (core [`txpack/chain`](https://github.com/jmandel/org.hl7.fhir.core/tree/txpack/chain), kindling [`perf/integrated`](https://github.com/jmandel/kindling/tree/perf/integrated)), driven by a same-machine run-to-run diff (two converged serial builds, race-free): the genuine run-to-run nondeterministic file set dropped from **21 to effectively 1** (only `fhir.ttl` remains; the rest of the residue is wall-clock timestamps and their container/manifest cascade). Fixes, each a minimal stabilization that leaves published content unchanged:

- **Root of the ordering churn (warnings.xml, toc.html section numbers, spec.internals targets):** `CanonicalResourceManager.allResources` was a `HashSet<CachedCanonicalResource>` whose element type has no `hashCode()` override, so identity-hash iteration order varied per JVM run and reordered every `getList()` consumer. Fixed `HashSet`→`LinkedHashSet` (loading order; `getSortedList()` still serves order-needing callers). This is the upstreamable core fix for source (1).
- **A 6th source, and a genuine correctness bug, not just nondeterminism:** `ValueSetExpander.addCode` appended its synthetic `preferredForLanguage` designation to the **caller's live (cached, shared) CodeSystem designation list**, so repeated expansions accumulated duplicates and the count flickered with processing order (visible in `expansions.json` and as a display-column appearing/empty in rendered value sets). Fixed with a defensive copy before mutating.
- **Image UUIDs (source 2, image half):** `PatientRenderer` named patient-photo files with a per-build random UUID (nondeterministic `<img src>` + uncleaned cruft). Fixed to a content-derived (CRC32) name.
- **kindling:** the NamingSystem registry bundle was emitted in `HashMap` keySet order (fixed: sort entries by fullUrl); `scanForPages`/`scanForImages` walked `File.listFiles()` in filesystem-enumeration order (fixed: sort by name) — this also addresses the cross-machine path-order class.

All verified behavior-safe (119 core unit tests green; both serial builds keep the exact Errors=0/Warnings=3693/Info=345 signature). **Remaining, honestly:** **(a)** `fhir.ttl` — the OWL ontology still reorders run-to-run (≈25 subjects via a secondary unsorted emission pass plus restriction/predicate ordering within subjects); a deeper `FhirTurtleGenerator`/RDF-model fix, the narrowed survivor of source (3)'s TTL class. **(b)** the table-script `// uuid` comment marker (source 2, table half) and the ShEx blocks (source 3) — still masked by the comparison normalizer rather than fixed at the generator. **(c)** source (4) the prior-output dependency — handled operationally by a convergence pass, not fixed in code. **(d)** for *parallel* (not just serial) reproducibility, the known 1-in-N search-param validation race (it flips validation results, e.g. spurious `ActivityDefinition.subject-*` errors) must also be fixed; serial validation sidesteps it. Net: a converged **serial** build is now reproducible run-to-run except for `fhir.ttl` and timestamps.
